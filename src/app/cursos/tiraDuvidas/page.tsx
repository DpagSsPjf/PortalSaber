...
export default async function DocumentacaoTiraDuvidasPage() {
  ...
  const sidebarItems = generateSidebarItems(tiraDuvidasData.capitulos);

  return (
    <div className="flex flex-1 pt-15">
      <SideBar title="Manual Tira Dúvidas" items={sidebarItems} />

      <article className="flex-1 p-4 md:pl-80 overflow-y-auto">
        {tiraDuvidasData.capitulos.map((capitulo) => (
          <section
            key={capitulo.id}
            id={capitulo.id}
